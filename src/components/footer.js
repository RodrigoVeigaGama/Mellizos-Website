  ...
  return `
    <footer class="site-footer">
      <div class="container footer-inner reveal reveal-delay-1">
        ...
        <div class="footer-legal"><p>© ${new Date().getFullYear()} ${site.name}</p><p>${site.responsible} — CNPJ ${site.cnpj}</p><p>${site.address}</p><p><a href="mailto:${site.email}">${site.email}</a> · <a href="${whatsappUrl()}" target="_blank" rel="noopener noreferrer">${site.whatsappDisplay}</a></p></div>
